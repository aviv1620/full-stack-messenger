...
export function ConnectionStatus() {
  const websocketStatus = useSelector((state) => state.connection.websocketStatus);

  const problem = websocketStatus !== WebSocket.OPEN;

  if (problem)
    return <div className="text-red-600">
      <svg xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 -960 960 960" width="48" className="w-full h-full fill-current">
